...
const RandomCall = () => {
    ...
    return (
        <div className={`w-full ${theme === "light" ? "bg-linear-to-b from-white to-emerald-700" : "bg-linear-to-b from-emerald-700 to-zinc-950"} h-screen flex justify-center items-center flex-col`}>
            ...
            <div className='buttons w-[95%] h-[10%] px-4 flex justify-between items-center gap-8'>
                <div className='time'>
                    <CallDuration peer={peerObj} />
                </div>
                <div className='buttons flex items-center justify-center gap-2'>
                    <div onClick={toogleCammera} className={`messageBtn ${theme === "light" ? "" : "text-gray-400"} glass transition-all p-3 rounded-full cursor-pointer`}>
                        {camOn ? <IoVideocam className='text-3xl' />
                            :
                            <IoVideocamOff className='text-3xl' />}
                    </div>
                    <div onClick={toggleMic} className={`messageBtn ${theme === "light" ? "" : "text-gray-400"} glass transition-all p-3 rounded-full cursor-pointer`}>
                        {micOn ? <IoMdMic className='text-3xl' />
                            :
                            <MdMicOff className='text-3xl' />}
                    </div>
                    <div onClick={endVideoCall} className={`messageBtn ${theme === "light" ? "" : "text-gray-400"} glass transition-all p-3 rounded-full cursor-pointer`}>
                        <MdCallEnd className='text-3xl text-red-600' />
                    </div>
                    <div onClick={nextCall} title='Next' className={`messageBtn ${theme === "light" ? "" : "text-gray-400"} glass transition-all p-3 rounded-full cursor-pointer`}>
                        <BsCaretRightFill className='text-3xl' />
                    </div>
                </div>
                <div className='flex gap-4 items-center justify-center'>
                    <div onClick={() => setIsOpenMessage((v) => ({...v, onClickIsOn: !v.onClickIsOn}))} className={`messageBtn ${theme === "light" ? "bg-gray-100 hover:bg-gray-200" : "hover:bg-gray-700 bg-gray-800 text-gray-400"} glass transition-all p-4 rounded-full cursor-pointer`}>
                        <FaMessage className='text-2xl' />
                    </div>
                    <div onClick={() => setTheme(theme === "light" ? "dark" : "light")} title='Next' className={`messageBtn ${theme === "light" ? "bg-gray-100 hover:bg-gray-200" : "hover:bg-gray-700 bg-gray-800 text-gray-400"} glass transition-all p-3 rounded-full cursor-pointer`}>
                        {theme === "light" ? <FaMoon className='text-3xl' title='dark' /> : <FaLightbulb className='text-3xl' title='light' />}
                    </div>
                </div>
            </div>
        </div>
